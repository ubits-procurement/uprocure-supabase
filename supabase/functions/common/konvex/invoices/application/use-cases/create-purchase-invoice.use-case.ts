...
export class CreatePurchaseInvoiceUseCase {
  ...
  async execute(
    ...
    const invoiceData: PurchaseInvoiceRequest = {
      ...
      createdFrom: receipt.data.createdFrom,
      currency: receipt.data.currency.id,
      entity: receipt.data.entity.id,
      subsidiary: receipt.data.subsidiary.id,
      number: props.tranId,
      dueDate: formattedDueDate,
      netsuite: {
        location: receipt.data.location?.id,
        custbody_ubits_grni: {
          id: receipt.data.id,
          refName: `Recepción de artículo #${receipt.data.tranId}`,
        },
        cseg_co_thirdparty: {
