...
for select using (
  ...
  exists (
    ...
    where provider_invoices_validations.provider_id = users.provider
  )
);
...
create policy "Usuarios admin pueden acceder a todos los archivos del bucket 'provider_invoices_validations'"
on storage.objects
for select using (
  bucket_id = 'provider_invoices_validations' AND
  exists (
